...
export class GroundComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get(this.csvPath,{responseType:'text'}).subscribe(next => {
      ...
      this.grounds = Array.from(new Set(this.data.map(grounds => (grounds.ground) )))
      .filter(c=> c!=undefined).sort();
      this.grounds.forEach(ground => this.totalScore.push(this.getBattingScoreVsGround(ground)));
      this.grounds.forEach(ground => this.centuriesground.push(this.getCenturies(ground,"100")));
      this.grounds.forEach(ground => this.halfCenturiesground.push(this.getCenturies(ground,"50")));
      this.grounds.forEach(ground => this.winGames.push(this.getWinGames(ground)));
      
      this.barChartLabels = this.grounds;
      this.barChartData = [{data:this.totalScore,label:"Score"}]

      this.barChartData1 = [ 
        {data: this.centuriesground,label: '100\'s'},
        {data: this.halfCenturiesground,label: '50\'s'}
      ]

      this.barChartData2 = [{data:this.winGames,label:"Matches Won"}]
